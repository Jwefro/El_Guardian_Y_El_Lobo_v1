...
const Page = () => {
  ...
  const handlePage = (ruta: string) => {
    ...
    setCurrentPage(`/prologo/6/sigilo/1/ataque-sorpresa/${ruta}`);
    setTimeout(() => {
      router.push(`/prologo/6/sigilo/1/ataque-sorpresa/${ruta}`); // Redirige a la página del juego después de la animación
          ...
          >
            <div className="">
              <Typography variant="p" className="pb-4">
                Decides actuar rápidamente, coordinando un ataque sorpresa junto
                a <span className="text-red-950 px-1">{wolfName}</span>. Desde
                tu posición oculta, observas cuidadosamente a los bandidos. Dos
                de ellos están más cerca de Lia y de la fogata, mientras que el
